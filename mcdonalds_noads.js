...
尽量不碰：订单、支付、优惠券、会员、菜单、门店、地址、价格等核心字段。
*/
...
function isCoreBusinessKey(key) {
  const k = lower(key);
  return /(order|payment|pay|coupon|voucher|member|login|token|cart|checkout|delivery|deliver|address|store|restaurant|menu|product|goods|sku|price|amount|invoice|pickup|takeout|takeaway|food|combo|meal|item|category|point|score|account|profile|phone|mobile|user)/.test(k);
}
...
function isAdString(value) {
  ...
  const v = lower(value);
  return /(splash|startup|openad|screenad|advert|advertise|advertisement|popup|popbox|banner|marketing|promotion|campaign|activitybanner|homebanner|topbanner|feedad|loadingpicture|interstitial|toastad|windowad|modalad)/.test(v);
}
...
function shouldDropObject(obj) {
  ...
  // 核心业务对象不整块删，只做字段级清理
  if (/(order|payment|pay|coupon|voucher|member|cart|checkout|menu|product|goods|sku|price|store|restaurant|address|delivery|pickup)/.test(keyText)) {
    return false;
  }
  ...
  // 弹窗、开屏、营销位对象
  if (/(splash|startup|openad|screenad|popup|popbox|pop|banner|float|marketing|promotion|advert|ad|campaign|activity|guide|modal|toast)/.test(keyText)) {
  ...
  if (obj && typeof obj === "object") {
    ...
    for (const k of Object.keys(obj)) {
      ...
      if (/(isshow|show|display|visible|enable|enabled|needshow|need_show|showflag|popupshow|adshow|isopen|openflag)/.test(lk)) {
        if (/(splash|startup|openad|screenad|popup|popbox|advert|ad|banner|marketing|promotion|campaign|activity|guide|modal|toast)/.test(snapshot)) {
          obj[k] = false;
        }
      }
