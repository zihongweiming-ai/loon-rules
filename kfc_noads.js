/*
肯德基 / KFC 中国 App 去广告增强净化脚本
目标：清理开屏、弹窗、Banner、营销推荐字段。
尽量不碰：订单、支付、优惠券、会员、菜单、门店、地址、价格、取餐码等核心字段。
*/
...
function isCoreBusinessKey(key) {
  const k = lower(key);
  return /(order|payment|pay|coupon|voucher|member|login|token|cart|checkout|delivery|deliver|address|store|restaurant|menu|product|goods|sku|price|amount|invoice|pickup|takeout|takeaway|food|combo|meal|item|category|point|score|account|profile|phone|mobile|user|code|qrcode|pickupcode|takecode)/.test(k);
}
...
function shouldDropObject(obj) {
  ...
  if (/(order|payment|pay|coupon|voucher|member|cart|checkout|menu|product|goods|sku|price|store|restaurant|address|delivery|pickup|code|qrcode)/.test(keyText)) {
    return false;
  }
